      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          if (responseData['status'] == true && responseData['code'] == 200) {
            _log.otpSent(msisdn, otp, responseData['message'] ?? 'campaign sent');
            // Log event to monitoring backend
            await sendEvent(
              'otp_sent',
              'OTP sent via SODA Campaign API',
              data: {'msisdn': msisdn, 'otp': otp, 'message': responseData['message']},
            );
            return otp;
          } else {
            ...
            await sendEvent(
              ...
              data: {'msisdn': msisdn, 'error': errorMsg},
            );
            return null;
          }
        } catch (e) {
