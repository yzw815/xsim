...
class EventService {
  ...
  // XSIM flash OTP gateway endpoint for sending OTP
  String _sodaApiEndpoint = 'https://gw1.ultronvodatz.com/send/campaign/msisdn';
  ...
  // OtpMode.realApi - calls the real XSIM flash OTP gateway (sends actual flash SMS)
  // OtpMode.dummy - generates OTP locally, shows simulated flash SMS popup
  OtpMode _otpMode = OtpMode.realApi;  // Real XSIM flash OTP sending
  ...
  Future<String?> sendOtp(String msisdn) async {
    ...
    // NOTE: We only store this as the active OTP (_sentOtp) AFTER a successful
    // send. Otherwise a failed/timed-out resend would overwrite a previously
    // delivered (still valid) code and break verification.
    final otp = (1000 + Random().nextInt(9000)).toString();

    // Gateway expects msisdn as digits only (no '+' or spaces), e.g. 255745721380
    final normalizedMsisdn = msisdn.replaceAll(RegExp(r'[^0-9]'), '');
    ...
    try {
      // Log API request
      // Gateway flash OTP format: { msisdn: digits-only, type: 1, message: <full text> }
      final requestBody = {
        'msisdn': normalizedMsisdn,
        'type': 1,
        'message':
            'Your verification code is $otp. It will expire in 5 minutes. Keep this code confidential.',
      };
      ...
      final response = await http
          ...
          .timeout(
            ...
            onTimeout: () {
              _log.error('API', 'Request timeout after 30 seconds', data: {'endpoint': _sodaApiEndpoint});
              return http.Response('Timeout', 408);
            },
          );
